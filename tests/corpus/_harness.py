...
The corpus (``corpus.jsonl``) is a JSON-lines file, one tournament per line::

    {"name": "ind_00000", "category": "individual", "valid": true,
     "skip": false, "skip_reason": null, "trf": "062 45\\n072 45\\n001 ..."}

Plain text rather than a compressed archive: a regeneration rewrites a few
hundred of the six thousand lines, which git stores as a delta against the
previous version and which ``git diff`` can be read against.  A recompressed
archive is a new blob every time and shows as one opaque change.
...
from gacrux import tiebreakchecker  # noqa: E402
