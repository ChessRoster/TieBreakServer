...
def tiebreak_status(trf_text):
    _write_trf(trf_text)
    return _drive(tiebreakchecker.tiebreakchecker, ["-t", "PTS"])


def engine_accepts(trf_text):
    """The engine accepts a tournament as valid exactly when its pairing check
    passes: the pairing it recomputes for every round matches the one the file
    declares.  A rejected tournament fails that check or makes the engine fault."""
    return pairing_status(trf_text) == 0
