...
* ``skip: true`` records (today, the whole ``team`` category, which the FIDE
  Swiss Team system is validated for separately) are skipped with their own
  ``skip_reason``.
...
def test_corpus_record(record):
    ...
    assert _harness.tiebreak_status(trf) != 510, \
        "tie-break checker faulted (status 510) on %s" % record["name"]

    # The engine's accept/reject verdict must match the corpus's `valid` label.
    accepts = _harness.engine_accepts(trf)
