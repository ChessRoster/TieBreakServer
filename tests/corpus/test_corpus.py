...
* records named in ``known_failures.json`` are marked ``xfail`` with the reason
  recorded there -- these tests still execute, but document a tournament the
  engine and the corpus disagree about. A disagreement is one of two things, and
  the reason recorded against the record says which:

  - a defect of the engine, not yet fixed; or
  - a fixture the engine has outgrown. The fixtures were generated by this
    engine, so correcting the engine moves fixtures out of agreement with it and
    the record that is now wrong is the fixture. ``tests/corpus/README.md``,
    "Fixtures the corrections outdated", derives every entry of that kind and
    says what makes the new answer the right one.

  ``strict=True`` in both cases: a record that starts passing turns into an XPASS
  and fails the suite, which is the signal to take it out of
  ``known_failures.json`` -- because the bug was fixed, or because the fixture
  was regenerated.
