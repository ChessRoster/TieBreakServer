...
to REPORT.md. See this directory's README.md, "Running the report" and
...
tests/corpus/corpus.jsonl by name and re-truncates it -- deterministic,
...
from engines.external_engine import BINARY_PATH, TIMEOUT_SECONDS, _blank_250_match_points  # noqa: E402

CORPUS = os.path.join(TESTS_DIR, "corpus", "corpus.jsonl")
...
def summarize(comparisons):
    ...
    for row in comparisons:
        ...
        # "Board order" is only a meaningful comparison between two PAIRED
        # outcomes that already agree on the pairs and colours -- MATCH and
        # COLOUR. For a PAIRING-class row the two engines produced different
        # boards outright, so `is_board_order_only_difference` answers False
        # for the same reason it would for a genuinely identical ordering:
        # "not order-only". Bucketing that False as "same board order"
        # reported a real pairing divergence as if it were a presentation
        # difference. PAIRABILITY and INCONCLUSIVE rows never had a raw
        # pairing on both sides to compare in the first place.
        bo = row.get("board_order_only_difference")
        if cls not in ("MATCH", "COLOUR"):
...
def _external_checklist(truncated_trf, round_no, out_dir):
    """Best-effort: run the external engine's -l flag for this round and save
    the checklist. Never raises -- this is a nice-to-have, not part of the
    reproducer's core content (the truncated TRF and both raw outcomes)."""
    if not os.path.exists(BINARY_PATH):
        return None
    trf, _ = _blank_250_match_points(truncated_trf)
    input_path = os.path.join(out_dir, "input.trf")
    output_path = os.path.join(out_dir, "external_pairing.out")
    checklist_path = os.path.join(out_dir, "external_checklist.txt")
...
def write_repro(group_key, rows, repro_dir):
    ...
    _external_checklist(truncated, rep["round"], out_dir)
...
def render(meta, skips, comparisons, by_variant, board_order, groups, repro_written, by_platform=None):
    ...
    lines.append("")
    lines.append("Generated by `tests/interop/report.py`. See `tests/interop/README.md` for the design.")
    lines.append("")
    ...
    if meta:
        ...
        lines.append("| external engine | `%s %s` |" % (meta.get("external_engine_name"), meta.get("external_engine_version")))
        lines.append("| external engine binary sha256 | `%s` |" % meta.get("external_engine_sha256"))
        lines.append("")
    ...
    lines.append("MATCH/COLOUR rows -- the only ones where the two engines already agree on")
    lines.append("the pairing itself, so \"board order\" is a meaningful question. Never gates")
    lines.append("MATCH/divergence -- see `tests/interop/README.md`'s \"Outcome model\".")
    lines.append("")
    ...
    lines.append("## Triage")
    lines.append("")
    lines.append("Not yet triaged here. Per `tests/interop/README.md`'s \"Triaging a divergence\",")
    lines.append("each reproducer above should be classified by hand into exactly one of")
    lines.append("`tiebreakserver is wrong`, `the external engine is wrong`, or `underdetermined`.")
    lines.append("")
